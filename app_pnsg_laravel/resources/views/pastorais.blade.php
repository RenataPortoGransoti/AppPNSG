            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
                        ...
                        @foreach ($pastorais as $pastoral)
                            @if ($pastoral->ativo)
                                <div class="bg-white shadow-lg rounded-2xl mb-4 mx-5">
                                    <div
                                        class="bg-[#9DDEFB] rounded-md py-3 px-4 flex justify-between items-center cursor-pointer border border-[#036896]">
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <button type="button" class="relative group btn btn-outline-danger pr-5"
                                                onclick="showExcluirModal({{ $pastoral->id }})">
                                                <i
                                    ...
                                    <div id="editForm{{ $pastoral->id }}"
                                        ...
                                        <form action="{{ route('pastorais.update', $pastoral->id) }}" method="POST"
                                            ...
                                            <div class="mb-4">
                                                ...
                                                @endif
                                                @if ($errors->has('photo'))
                                                    <p class="text-red-500 text-sm">{{ $errors->first('photo') }}</p>
                                                @endif
                                            </div>
                ...
                <div id="cardPastoral" class="my-4 hidden">
                    <div class="bg-white rounded-2xl shadow-lg">
                        ...
                        <form action="{{ route('pastorais.store') }}" method="POST" enctype="multipart/form-data"
                            class="p-4" id="formPastoral">
                            @csrf
                            <div class="mb-6">
                                <label for="nomePastoral" class="form-label mb-1 block">Nome da Pastoral*</label>
                                <input type="text" class="px-2 form-input w-full border rounded-md h-12"
                                    id="nomePastoral" name="nomePastoral" maxlength="60" required>
                                <div id="" class="error-message text-red-500 text-sm" style="display: none">Por
                                    favor, preencha o nome da pastoral.</div>
                            </div>
                            <div class="mb-6">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="file" name="photo" id="photo"
                                    class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-black focus:border-black">
                                <p class="text-sm mx-2">Formatos aceitos (JPG | JPEG| PNG). Tamanho máximo: 2MB.</p>
                            </div>
                    ...
                    <div class="bg-white w-1/3 p-8 rounded-md">
                        ...
                        <div class="flex justify-center items-center align-center">
                            <button id="btnExcluirFecharModal"
                                class="px-4 py-2 bg-[#036896] hover:bg-[#9DDEFB] hover:text-black text-white border border-[#036896] rounded-xl mr-2">Voltar</button>
                            <form id="formExcluirPastoral" method="POST"
                                action="{{ route('pastorais.inativar', ['id' => 0]) }}">
                                @csrf
